...
import { shuffle, map, getMaxScore, findObjectInListByTag, getScore } from '../../tools/functions';
...
export const GameScreen = ({ route, navigation, ...props }) => {
  ...
  const handleRecordPress = () => navigation.navigate('MedicalRecord', { data: gameData, scores: scores });
  ...
  function tryStartInterference(force) {
    ...
    if (gameData.interference) {
      if (gameData.interference.etapa === etapa.index) {
        ...
        const shouldOccure = gameData.interferenceState === 1 || force || randomChance;
        if (shouldOccure) {
          if (gameData.interference.responses_until_start === 0 || force) {
            navigation.navigate('Interference', {
              ...
              backgroundImage: findObjectInListByTag(mCaso.imagens, 'identificador',
              ...
              backgroundImagePatient: gameData.interference.key !== "PACIENTE_DESCONTROLE_EMOCIAL" && findObjectInListByTag(mCaso.imagens, 'identificador', gameplayScreenplay.etapas[etapa.index].patient_image).arquivo,
    ...
    if (etapa.index === 3 || etapa.index === 5 || etapa.index === 7) {
      ...
      if (etapa.index === 7) {
        if (gameData.selections.comunicacao) {
          scores[etapa.key].score += getScore(option, etapa.key) - getScore(gameData.selections.comunicacao, etapa.key);
        } else {
    ...
    if (interfStart) {
      if (gameData.interference.forced_response) {
        if (gameData.interference.responses_until_start > 0) {
          ...
          if (option.feedback) {
            const text = gameData.interference.key === "RUIDOS_EXTERNOS"
  ...
  const handleNextStep = () => {
    ...
    if (gameData.interference) {
      ...
      } else if (gameData.interference.etapa === index && gameData.interferenceState !== 2) {
        tryStartInterference(true);
        return;
      }
    }
    ...
    if (index === 7 || index === 5 && gameData.selections.diagnostico?.tipo !== "+") {
      handleEndGame();
      return;
    }
  ...
  const isSpeech = speechControl.enableOptions
    ...
    : gameplayScreenplay.etapas[etapa.index].falas[speechControl.pagination].is_speech;
  ...
  return (
    <Container containerStyle={{ backgroundColor: 'black' }}>
      <ImageBackground style={theme.styles.flex} source={{ uri: findObjectInListByTag(mCaso.imagens, 'identificador', gameplayScreenplay.etapas[etapa.index].background_image).arquivo }} resizeMode="contain">
        <ImageBackground style={theme.styles.flex} source={{ uri: findObjectInListByTag(mCaso.imagens, 'identificador', gameplayScreenplay.etapas[etapa.index].patient_image).arquivo }} resizeMode="contain">
          {/* Ícone de prontuário (direita) */}
          <CircleButton size={theme.measure(5)} style={{ position: 'absolute', top: theme.measure(2), left: theme.measure(2) }} onPress={handleRecordPress}>
            <Image source={MedicalRecordImage} style={{ height: theme.measure(3), width: theme.measure(3) }} resizeMode='contain' />
          </CircleButton>

          {/* Ícones brancos da esquerda */}
          <View style={[theme.styles.spacedRow, { position: 'absolute', top: theme.measure(2), right: theme.measure(7) }]}>
            <CircleButton size={theme.measure(3)} style={{ marginRight: theme.measure(1) }}>
              <Image source={HelpImage} style={{ height: theme.measure(2), width: theme.measure(2) }} resizeMode='contain' />
            </CircleButton>
            <CircleButton size={theme.measure(3)} style={{ marginRight: theme.measure(1) }} onPress={handleMenu}>
          ...
          {feedbackContent.visible && (
            <View style={[theme.styles.feedbackBox, { backgroundColor: feedbackContent.isSpeech ? '#ACDCCE' : appColors.cardGray }]}>
          ...
          <SpeechThink
            arrowBlink={(speechControl.pagination === 0 && !speechControl.enableOptions) || (etapa.index === 3 && gameData.selections.diagnostico)}
            isSpeech={isSpeech}
            onArrowClick={handlePaginationPress}
            onOptionPress={handleOptionPress(options.perPage * options.pagination)}
            options={options.options?.slice(options.perPage * options.pagination, options.perPage * (1 + options.pagination))}
            ...
            showArrow={(options.options.length > options.perPage) || !speechControl.enableOptions || (etapa.index === 3 && gameData.selections.diagnostico)}
            showText={!speechControl.enableOptions}
            text={gameplayScreenplay.etapas[etapa.index].falas[speechControl.pagination].text}
          />
          ...
          <View style={[theme.styles.spacedRow, { position: 'absolute', bottom: 0, left: 0, right: 0, alignItems: 'flex-end' }]}>
            <StepButton style={{ flexGrow: 1 }} title={'Anamnese'} step={1} currentStep={etapa.index}
              onPress={handleNextStep}
            />
            <StepButton style={{ flexGrow: 1 }} title={'Exame Clínico'} step={2} currentStep={etapa.index}
              onPress={handleNextStep}
            />
            <StepButton style={{ flexGrow: 1 }} title={'Exames Complementares'} step={4} currentStep={etapa.index}
              onPress={handleNextStep}
            />
            <StepButton style={{ flexGrow: 1 }} title={'Diagnóstico'} step={5} currentStep={etapa.index}
              onPress={handleNextStep}
            />
            <StepButton style={{ flexGrow: 1 }} title={'Conduta'} step={6} currentStep={etapa.index}
              onPress={handleNextStep}
            />
            <StepButton style={{ flexGrow: 1 }} title={'Comunicação'} step={7} currentStep={etapa.index}
              onPress={handleNextStep}
            />
          </View>
        </ImageBackground>
